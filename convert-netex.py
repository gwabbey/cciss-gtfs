...
class NeTExExtractor:
    ...
    def extract(self, xml_path: Path):
        ...
        targets = {ns("ScheduledStopPoint"), ns("Line"), ns("Operator"), ns("Authority"), ns("ServiceJourneyPattern"),
                   ns("ServiceJourney"), ns("ServiceCalendar"), ns("DayTypeAssignment"),
                   ns("UicOperatingPeriod"), ns("OperatingPeriod"), }
        ...
        LOG.info(f"Found: {len(self.stops)} stops, {len(self.lines)} lines, "
                 f"{len(self.patterns)} patterns, {len(self.journeys)} journeys, "
                 f"{len(self.service_links)} service links, "
                 f"{len(self.operating_periods)} operating periods")
...
def convert(extractor: NeTExExtractor, writer: GTFSWriter, make_shapes: bool = True):
    ...
    pattern_shapes = {}
    if make_shapes and extractor.service_links:
        LOG.info("Building shapes from ServiceLinks...")
        shape_counter = 0

        for pid, stops in extractor.patterns.items():
            shape_points = []

            for order, stop_id, link_id in stops:
                if stop_id in extractor.stops:
                    _, lat, lon = extractor.stops[stop_id]
                    shape_points.append((float(lat), float(lon)))

                if link_id and link_id in extractor.service_links:
                    shape_points.extend(extractor.service_links[link_id])

            if len(shape_points) >= 2:
                shape_counter += 1
                shape_id = f"shp_{shape_counter}"
                pattern_shapes[pid] = shape_id

                for seq, (lat, lon) in enumerate(shape_points, 1):
                    writer.write("shapes.txt",
                                 {"shape_id": shape_id, "shape_pt_lat": f"{lat:.6f}", "shape_pt_lon": f"{lon:.6f}",
                                  "shape_pt_sequence": seq})

        LOG.info(f"Created {shape_counter} shapes")
    elif make_shapes:
        LOG.info("Shape generation enabled but no ServiceLinks found in NeTEx")
    else:
        LOG.info("Shape generation disabled")
    ...
    for jid, pid, headsign, daytype_id, journey_name in extractor.journeys:
        ...
        shape_id = pattern_shapes.get(pid, "")

        line_info = extractor.lines.get(route_id)
        trip_short_name = ""
        ...
        writer.write("trips.txt",
                     {"route_id": route_id, "service_id": service_id, "trip_id": jid, "trip_headsign": headsign,
                      "trip_short_name": trip_short_name, "shape_id": shape_id})
        trips_written += 1
...
def main():
    ...
    for entry in entries:
        ...
        make_shapes = entry.get("make_shapes", True)

        if not url or not output:
            LOG.error("Skipping invalid entry: %s", entry)
            continue

        LOG.info("Processing %s → %s (shapes=%s)", url, output, make_shapes)

        try:
            convert_single(url, out / output, make_shapes)
        except Exception as e:
            LOG.exception("Failed to convert %s", url)


def convert_single(url: str, out_path: Path, make_shapes: bool = True):
    with TemporaryDirectory(prefix="netex_") as tmpdir:
        ...
        extractor = NeTExExtractor(parse_service_links=make_shapes)
        extractor.extract(xml_path)

        gtfs_dir = tmpdir / "gtfs"
        writer = GTFSWriter(gtfs_dir)

        convert(extractor, writer, make_shapes=make_shapes)
        writer.close()
